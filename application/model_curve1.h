...
namespace hbb {
  ...
  template <typename T>
  class Model_curve1 : public GMlib::PCurve<T,3> {
   ...
   */
    template <typename T>
    inline
    Model_curve1<T>::Model_curve1() : GMlib::PCurve<T,3>(20, 0, 1) {
    ...
    template <typename T>
    inline
    Model_curve1<T>::Model_curve1( const Model_curve1<T>& copy ) : GMlib::PCurve<T,3>(copy) {}
